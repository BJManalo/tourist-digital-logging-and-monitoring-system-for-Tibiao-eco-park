...
const DISCOUNTED_STATUSES = ['PWD', 'Senior Citizen'];
...
function calculateTotalValue() {
    ...
    const isPrimaryDiscounted = DISCOUNTED_STATUSES.includes(primaryStatus);
    const baseFee = FEES[primaryType] || 50;
    const primaryCost = isPrimaryDiscounted ? baseFee * (1 - DISCOUNT_RATE) : baseFee;
    total += primaryCost;
    ...
    memberCards.forEach((card) => {
        ...
        const isDiscounted = DISCOUNTED_STATUSES.includes(status);
        const mBaseFee = FEES[type] || 50;
        const cost = isDiscounted ? mBaseFee * (1 - DISCOUNT_RATE) : mBaseFee;
        total += cost;
    });
...
function renderPreview(formData) {
    ...
    const isDiscounted = DISCOUNTED_STATUSES.includes(formData.status_display);
    ...
    formData.members.forEach(m => {
        ...
        const mIsDiscounted = DISCOUNTED_STATUSES.includes(m.status_display);
